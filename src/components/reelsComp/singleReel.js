  ...
  useEffect(() => { 
     ...
     const prefetchNextVideo = async () => { 
       if (currentIndex === index + 1 && nextVideoUrl) { 
         try { 
           const compressedUrl = await CompressorVideo.compress(nextVideoUrl, { 
             compressionMethod: 'auto', 
           }); 
           setIsLoading(false);  
           Video.prefetch(compressedUrl);  
         } catch (error) { 
           console.log('Error compressing and prefetching next video:', error); 
           setIsLoading(false);  
         } 
       } 
     }; 
